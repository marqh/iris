...
def _ensure_save_rules_loaded():
    ...
    if _save_rules is None:
        ...
        rules_filename = os.path.join(iris.config.CONFIG_PATH, 'grib_save_rules.txt')
...
def save_grib2(cube, target, append=False, field_coords=None):
    """
...
#        grib_save_rules.run(slice2D, grib_message)
        grib_message = gribapi.grib_new_from_samples("GRIB2")
        rules_result = _save_rules.verify(slice2D, grib_message)
